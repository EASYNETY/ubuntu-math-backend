/**
 * FORCE UPLOAD BOOKS - Bypasses duplicate check
 * =============================================
 * This script DELETES all existing book records and re-uploads everything fresh.
 * Use this when the duplicate detection is causing issues.
 */
...
async function main() {
  console.log('\n🚀 FORCE UPLOAD BOOKS (Bypassing duplicate check)');
  console.log('='.repeat(60));
  console.log('⚠️  WARNING: This will DELETE all existing book records!');
  console.log('='.repeat(60));
  ...
  console.log('\n🗑️  Deleting all existing book records...');
  const deleteResult = await PlatformContent.deleteMany({ contentType: 'book' });
  console.log(`✅ Deleted ${deleteResult.deletedCount} existing book records`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (!meta) {
      ...
      results.failed.push({ file: filename, error: 'No metadata' });
      continue;
    }
    ...
    try {
      ...
      if (sampleText && sampleText.length > 100) {
        ...
        console.log(`  ✅ Sample chapters uploaded`);
      } else {
      ...
      console.log(`  ✅ Database record created (ID: ${newBook._id})`);
